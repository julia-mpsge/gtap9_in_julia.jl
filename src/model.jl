...
Return the GTAP9 model as an `MPSGEModel`. The input `data` should be the dictionary
returned by `load_data`.  

## Model Discussion

This model uses indexed nests, which is a fairly advanced feature of MPSGE and 
requires a little discussion. In the production block for the `Y` sector, we have
the nest `nest[i=I]=>m=esubdm[i]`. This generates a nest called `nest` for each
`i` in `I`. The nest `nest[i]` sits below `m` and has elasticity of substitution
`esubdm[i]`.  

The nest `nest[i]` gets used in the following two inputs:
...
This combines the price and margins of a good into separate nests. 
